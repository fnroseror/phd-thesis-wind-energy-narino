...
# Construcción de cache horario por zona para WPD
...
QC_FILE    <- file.path(BASE_DIR, "SALIDAS_PREPROCESSING", "01_QC", "07_Datos_Largos_QC.txt")
OUT_DIR    <- file.path(BASE_DIR, "SALIDAS_PREPROCESSING", "02_WPD_CACHE")
OUT_FILE   <- file.path(OUT_DIR, "WPD_hourly_por_zona.csv")
LOG_FILE   <- file.path(OUT_DIR, "02_build_hourly_wpd_cache_log.txt")
...
# Nos quedamos con las variables necesarias para construir WPD
DT <- DT[Variable %in% c("VV", "PA", "TM", "TMIN", "TMAX")]

if (nrow(DT) == 0) {
  stop("No quedaron datos útiles para construir WPD.")
}
...
# ============================================================
# 6. CONSTRUCCIÓN DE VARIABLES FÍSICAS
# ============================================================
...
W[!(is.finite(WPD) & WPD >= 0 & rho > 0.5 & rho < 2.0), WPD := NA_real_]

# ============================================================
# 7. RESÚMENES DE TRAZABILIDAD
# ============================================================
...
RES_ZONA <- W[, .(
  ...
  wpd_sd = sd(WPD, na.rm = TRUE)
), by = Zona]

fwrite(RES_ZONA, file.path(OUT_DIR, "01_resumen_wpd_por_zona.csv"))
...
RES_GLOBAL <- data.table(
  ...
  wpd_sd = sd(W$WPD, na.rm = TRUE)
)

fwrite(RES_GLOBAL, file.path(OUT_DIR, "02_resumen_global_wpd_cache.csv"))

# ============================================================
# 8. EXPORTACIÓN DEL CACHE
# ============================================================
