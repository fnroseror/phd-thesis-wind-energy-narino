...
# CAPÍTULO 3 - TDQ–PIESS/KFAS (WPD + I_TDQ + PI90)
# I_TDQ is internal to the hourly pipeline and is not the Chapter 4 FNRR.
...
BASE_FILE <- TDQ_DATA_FILE
stopifnot(file.exists(BASE_FILE))

BASE_DIR <- TDQ_WORK_DIR
...
wide[, I_TDQ := 1 / (1 + exp(-GAMMA_I_TDQ * U_z))]
wide[!is.finite(I_TDQ), I_TDQ := 0.5]
...
diag_zone <- wide[, .(
  ...
  N_FNRR = sum(is.finite(I_TDQ)),
  FNRR_p10 = quantile(I_TDQ, 0.10, na.rm = TRUE),
  FNRR_p50 = quantile(I_TDQ, 0.50, na.rm = TRUE),
  FNRR_p90 = quantile(I_TDQ, 0.90, na.rm = TRUE)
), by = Zona]

fwrite(diag_zone, file.path(TAB_DIR, "TDQ_DIAGNOSTICO_FILAS_POR_ZONA.csv"))

wide <- wide[is.finite(WPD) & is.finite(I_TDQ)]
...
run_zone_h <- function(dfz, h, horizon_name) {
  ...
  preds <- data.table(
    ...
    c_PI90 = c_PI90,
    I_TDQ = df$I_TDQ[idx_te],
    FNRR_mean = mean(df$I_TDQ[idx_te], na.rm = TRUE)
  )
...
p_fnrr <- ggplot(wide, aes(x = I_TDQ)) +
  geom_density() +
  facet_wrap(~Zona, nrow = 2) +
  labs(title = "I_TDQ | Distribución por zona", x = "I_TDQ", y = "Densidad") +
  theme_minimal(base_size = 12)
save_plot(p_fnrr, "05_I_TDQ_density_by_zone")
