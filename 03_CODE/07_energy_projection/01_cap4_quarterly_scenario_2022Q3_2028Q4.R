...
# CAPÍTULO 4 — ESCENARIO TRIMESTRAL SEPARADO Y FNRR
# WPD + indicadores E_free/E_usable + diagnóstico PI90 + FNRR
...
fnrr_from_p <- function(p50, p90, W0 = TDQ_W0, eps = TDQ_EPS) {
  p90_star <- pmax(p90, p50)
  a <- log1p(p90_star / W0)
  b <- log1p(p50 / W0)
  clamp01((a - b) / (a + eps))
}
...
p1 <- ggplot(tri_plot, aes(t_index, WPD_mean, color = factor(Zona), group = factor(Zona))) +
  ...
  labs(
    title = "WPD trimestral: histórico y escenario separado 2022-Q3–2028-Q4",
...
p2 <- ggplot() +
  ...
  labs(
    title = "Indicadores anuales E_free y E_usable con diagnóstico PI90",
    x = "Año",
    y = "Indicador integrado (kWh/m²)",
    color = "Zona",
    fill = "Zona"
  ) +
  theme_pub +
...
p3 <- ggplot(ann, aes(Year, FNRR, color = factor(Zona), group = factor(Zona))) +
  ...
  labs(
    title = "FNRR anual de la etapa trimestral separada (2017–2028)",
    x = "Año",
    y = "FNRR (0–1)",
    color = "Zona"
  ) +
